...
class EspeakDriver(object):
    ...
    def set_hardware(self, device_name):
        if device_name in self.hardware_memory:
            ret = self.hardware_memory[device_name]
            self.hardware = ret
            return
        ...
        def get_device_id(device_name):
            ...
            for line in lines:
                if device_name in line:
                    ...
                    device_number = line.split(':')[1].split(' ')[-1]
                    break
        
            if card_id is not None and device_number is not None:
                return f"hw:{card_id},{device_number}"
            else:
                rospy.logerr(f'Failed to get device id for {device_name}. Using default device {self.hardware}')
                return 'hw:1,0'
        self.hardware_memory[device_name] = get_device_id(device_name)
        self.hardware = self.hardware_memory[device_name]
                ...
                try:
                    ...
                    if event.user_data:
                        ...
                        if platform.system() == 'Darwin':  # macOS
                            ...
                            os.system(f'aplay -D plug{self.hardware} {stream.name} -q')
